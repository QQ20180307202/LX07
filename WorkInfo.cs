...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public static WorkInfo _instance = new WorkInfo();
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activityName fron workinfo join activity on workinfo.activitId=activity.activityld where workId=@id";
        ...
        public IEnumerable<Model.WorkInfo> GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select *from workinfo where workVerify='审核通过’order by uploadime desc limit 8";
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过’and recommend='是’order by reconmendTline desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row nunber () over (order by uploadrine desc) as num,workinfo.* from workinfo where activityId in activitIds)";
                sql += "select* from a where num between (@pageIndex-1)*@pageSize+1 and @pageIndex *@pageSize ; ";
                return cn.Query<Model.WorkInfoNo>(sql, new { pageIindex = page.pageIndex, pageSize = page.pageSize, ativityIds = page.activityIds });
            }
        }
        public int GetFindCount(string workName)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count (1) fron workinfo where workverify='审核通过’and worklName like concat ( '%', @worName, '%')";
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId workname, workPicture, uploadTime, workIntroduction,workVerity, username, activityId,recommend, recommendTime)" + "values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workNmae=@workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," + "userName=@userName recommend=@recommend, recommendTime=@recommendTime where workId=@workTd";
        ...
        public int UpdateRecommend(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set recommend=@recomnend, recommendTime=@recommendTime where workId=@workId";
                return cn.Execute(sql, workinfo);
            }
        }
    }
